...
use Grazulex\AutoBuilder\BuiltIn\Triggers\OnLogout;
...
describe('metadata', function () {
    it('has correct metadata', function () {
        $brick = $this->registry->resolve(OnLogout::class);

        expect($brick->name())->toBe('User Logout');
        expect($brick->type())->toBe('trigger');
        expect($brick->category())->toBe('Authentication');
        expect($brick->icon())->toBe('log-out');
    });

    it('has correct description', function () {
        $brick = $this->registry->resolve(OnLogout::class);

        expect($brick->description())->toContain('logs out');
    });
...
describe('sample payload', function () {
    it('returns sample payload with required keys', function () {
        ...
        expect($payload)->toHaveKey('logged_out_at');
    });
